...
res <- stan(
    file = file.path("stan_code", "ideal_points_2d.stan"),
    data = list(
        n_rows = n_endorsers,
        n_cols = n_candidates,
        n_obs = length(v),
    ...
    init = list(
        list(
            ...
            x1 = rnorm(n_endorsers, 0, 1),
            x2 = rnorm(n_endorsers, 0, 1),
            b = logit(bound_probs(colMeans(M, na.rm = TRUE))),
            y1 = rnorm(n_candidates, 0, 1),
            y2 = rnorm(n_candidates, 0, 1)
        )
    )
)
...
x1_inds <- paste0("x1[", 1:n_endorsers, "]")
x2_inds <- paste0("x2[", 1:n_endorsers, "]")
y1_inds <- paste0("y1[", 1:n_candidates, "]")
y2_inds <- paste0("y2[", 1:n_candidates, "]")

# Store ideal points for endorsers.
ideal_points_endorsers <- data.frame(
    endorser = wide_endorsements[, 1],
    mean1 = params[x1_inds, 1],
    lower1 = params[x1_inds, 4],
    upper1 = params[x1_inds, 8],
    mean2 = params[x2_inds, 1],
    lower2 = params[x2_inds, 4],
    upper2 = params[x2_inds, 8],
    stringsAsFactors = FALSE
)
...
write.csv(
    ...
    file = file.path("ideal_points_2", "endorsers.csv"),
    row.names = FALSE
)
...
p <- ggplot(
    ideal_points_endorsers,
    aes(x = reorder(endorser, mean1), y = mean1)
) +
    geom_point() +
...
p <- ggplot(
    ideal_points_endorsers,
    aes(x = mean1, y = mean2, label = endorser)
) +
    geom_text() +
    theme_bw() +
    theme(text = element_text(family = "mono"))
...
ggsave(
    file.path("ideal_points_2", "endorsers.png"),
    height = 14,
    width = 10
)
...
ideal_points_candidates <- data.frame(
    ...
    mean1 = params[y1_inds, 1],
    lower1 = params[y1_inds, 4],
    upper1 = params[y1_inds, 8],
    mean2 = params[y2_inds, 1],
    lower2 = params[y2_inds, 4],
    upper2 = params[y2_inds, 8],
    stringsAsFactors = FALSE
)
...
write.csv(
    ...
    file = file.path("ideal_points_2", "candidates.csv"),
    row.names = FALSE
)
...
# Save the plot to a PNG file.
ggsave(
    file.path("ideal_points_2", "dccc.png"),
    height = 14,
    width = 30
)
...
# Save the plot to a PNG file.
ggsave(
    file.path("ideal_points_2", "props.png"),
    height = 10,
    width = 30
)
